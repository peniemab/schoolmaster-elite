...
import TrimestreSelector from '@/components/Trimestreselector'; // Vérifie bien le chemin
// On utilise ton vrai fichier de config maintenant !
...
export default function EleveView() {
    ...
    const chargerEleves = async () => {
      ...
      try {
        ...
        const resultatsFiltrés = response.documents.filter(doc => {
          ...
          // Utilisation des vraies valeurs de ta config
          if (trimestre === 0) { 
  ...
  // --- LOGIQUE IMPRESSION : LISTE PROF (A4 Portrait) ---
  ...
  // --- LOGIQUE IMPRESSION : RECOUVREMENT (A4 Landscape) ---
  const imprimerRecouvrement = () => {
    ...
    y += 10;
    eleves.forEach((e, i) => {
      // Calcul du reste avec TOUS les frais de ta config
      ...
      // Utilisation de formatPrix pour que ce soit beau
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-2xl font-black text-slate-800 mb-6">Liste des Élèves</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div className="flex gap-2">
              <select value={classe} onChange={(e) => setClasse(e.target.value)} className="w-1/2 p-4 bg-slate-50 rounded-2xl outline-none text-sm font-bold text-slate-600">
                ...
                {[1,2,3,4,5,6].map(n => <option key={n} value={n.toString()}>{n}ème Année</option>)}
              </select>
          ...
          <div className="flex gap-3 mt-6">
            {['Tous', 'À jour', 'Impayés'].map((s) => (
              <button 
                ...
                className={`px-6 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${filtreStatut === s ? 'bg-blue-600 text-white shadow-lg' : 'bg-slate-100 text-slate-400'}`}
              >
                {s}
              </button>
            ))}
        ...
        <div className="flex justify-end gap-4 mb-4">
          <button onClick={imprimerListeProf} className="flex items-center gap-2 bg-slate-800 text-white px-5 py-3 rounded-2xl text-xs font-bold hover:bg-black transition-all">
            <Printer size={16} /> Liste Prof
          </button>
          <button onClick={imprimerRecouvrement} className="flex items-center gap-2 bg-blue-700 text-white px-5 py-3 rounded-2xl text-xs font-bold hover:bg-blue-800 transition-all">
            ...
            <table className="w-full text-left border-collapse">
              <thead className="bg-slate-50 border-b border-slate-100">
                <tr>
                  ...
                  <th className="p-4 text-[10px] font-black text-slate-400 uppercase text-center">Sexe</th>
                ...
                {eleves.map((e) => (
                  <tr key={e.$id} className="border-b border-slate-50 hover:bg-slate-50/50">
                    <td className="p-4">
                      ...
                      <p className="text-[10px] text-slate-400 font-mono">{e.$id.substring(0, 8).toUpperCase()}</p>
                    </td>
                    <td className="p-4 text-center font-bold text-slate-600 text-sm">{e.classe}ème {e.section}</td>
                    <td className="p-4 text-center text-slate-500 text-sm">{e.sexe}</td>
                    <td className="p-4 text-right">
                      <button className="text-blue-600 font-bold text-xs">Détails</button>
                    </td>
                  </tr>
                ))}
              </tbody>
